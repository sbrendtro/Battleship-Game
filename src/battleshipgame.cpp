...
battleshipGame::battleshipGame ():Destroyer (4), Carrier (5)
{
}
...
void
battleshipGame::introduction () const
{
...
void
battleshipGame::initialize (int lev)
{
  cout << "Intializing level: " << lev << ". It will commence shortly." <<
    endl;
  ...
  while (sent != 0)
    {
      if (theBoard.
	  placeship (Carrier.setx (), Carrier.sety (), Carrier.setHV (),
		     Carrier.getsize (), lev, 'C'))
	sent = 0;
    }
  ...
  while (sent != 0)
    {
      if (theBoard.
	  placeship (Submarine.setx (), Submarine.sety (), Submarine.setHV (),
		     Submarine.getsize (), lev, 'S'))
	sent = 0;
    }
  ...
  while (sent != 0)
    {
      if (theBoard.
	  placeship (Destroyer.setx (), Destroyer.sety (), Destroyer.setHV (),
		     Destroyer.getsize (), lev, 'D'))
	sent = 0;
    }
