...
main ()
{
  ...
  do
      ...
      while (abjects.ammo > 0 && abjects.hits < 12)
	{
	  cout << endl
	    <<
	    "Insert your hit. Letter first (column), and then the number (row) WITHOUT A SPACE for double digits "
	    << endl;
	  //cin>> abjects;
	  cin.clear ();
	  //cin.ignore();
...
whileCheck (char &a)
{
  ...
  while (a != 'y' && a != 'Y' && a != 'n' && a != 'N')
    {
      cout <<
	"Did you enter the correct response? If not, please enter your response again."
	<< endl;
      cin.clear ();
      cin.ignore (100, '\n');
      cin >> a;
